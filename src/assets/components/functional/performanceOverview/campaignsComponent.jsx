...
const CampaignsComponent = (props, ref) => {
    ...
    const clearCampaignCaches = () => {
        try {
            ...
            for (let i = 0; i < localStorage.length; i++) {
                ...
                if (key && key.includes('/bunge/campaign')) {
            ...
            renderCell: (params) => <BudgetCell
                ...
                onUpdate={async (campaignId, newBudget) => {
                    ...
                    try {
                        ...
                        let url = `https://react-api-script.onrender.com/bunge/campaign?start_date=${startDate}&end_date=${endDate}&platform=${operator}${ts}`;
    ...
    const CampaignsColumnFlipkart = [
        ...
        },
        {
            ...
            minWidth: 155,
            renderCell: (params) => (
                <ValueFormatter value={params.row.remaining_budget} />
            ), type: "number", align: "left",
            headerAlign: "left",
        },
        ...
        },
        {
            ...
            headerName: "CONVERTED UNITS % CHANGE",
            hideable: false
        },
        ...
        },
        {
            ...
            minWidth: 150,
            renderCell: (params) => (
                <NewPercentageDataComponent firstValue={params.row.ctr_y} secValue={params.row.ctr_diff} />
            ), type: "number", align: "left",
            headerAlign: "left",
        },
        {
        ...
        },
        {
            ...
            minWidth: 150,
            renderCell: (params) => (
                <NewPercentageDataComponent firstValue={params.row.acos} secValue={params.row.acos_diff} />
            ), type: "number", align: "left",
            headerAlign: "left",
        },
        {
    ...
    const handleCampaignClick = async (campaignName, campaignId) => {
        try {
            ...
            const startDate = formatDate(dateRange[0].startDate);
            const endDate = formatDate(dateRange[0].endDate);
            let url = `https://react-api-script.onrender.com/bunge/campaign_graph?start_date=${formatDate(startDate)}&end_date=${formatDate(endDate)}&platform=${operator}&campaign_id=${campaignId}`;
            if (selectedBrand && selectedBrand.trim() !== "") {
                url += `&brand_name=${encodeURIComponent(selectedBrand)}`;
            }
            const cacheKey = `cache:GET:${url}`;
    ...
    const confirmStatusChange = async (campaignId, newStatus, adType) => {
        try {
            ...
            const playPauseUrl = `https://react-api-script.onrender.com/bunge/play-pause`;
                    ...
                    <DialogActions>
                        ...
                        <Button color="primary" onClick={async () => {
                            ...
                            try {
                                ...
                                const response = await fetch("https://react-api-script.onrender.com/bunge/play-pause", {
                                    method: "PUT",
                                    headers: {
                                        "Content-Type": "application/json",
                                        Authorization: `Bearer ${token}`,
